        ...
        <div className='p-4'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-8'>
                {liveStreams
                    .sort(
                        (a, b) =>
                            b.availableAt.getTime() - a.availableAt.getTime(),
                    )
                    .map((stream) => (
                        <StreamItem key={stream.id} video={stream} />
                    ))}
            </div>
            ...
            {/* The Sentinel Element for IntersectionObserver to watch */}
            <div ref={observerTarget} className="w-full flex justify-center p-4 mt-4 h-12">
                {isLoading && <p className="text-gray-500 font-medium">Loading more streams...</p>}
            </div>
        </div>
    );
}
